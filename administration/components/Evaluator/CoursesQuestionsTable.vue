  ...
  <v-main>
    ...
    <v-card v-if="dialogAction==='Show'">
      <v-card-title>
        ...
        <v-icon
          v-if="questionToHandle.creator === $auth.user.email"
          small
          @click="editQuestion()"
        >
          mdi-pencil
...
export default {
  name: "CoursesQuestionsTable",
  components: {AddQuestion, ShowQuestion, EditQuestion},
  data() {
    return {
      dialogAction: "Show",
      dialog: false,
  ...
  methods: {
    ...
    async getQuestions() {
      this.loading = true
      try{
        const questions = await this.$axios.post('eval/get-questions-from-courses', {
