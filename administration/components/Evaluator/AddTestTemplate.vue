<template>
  <div>
    <validation-observer
      ref="observer"
      v-slot="{ handleSubmit }">
      <form @submit.prevent="handleSubmit(addTestTemplate)">
...
export default {
  ...
  data() {
    return {
      topics: [],
      template: [
        {topic: "", questionNumber: 0, questionValue: 0}
      ],
      title: "",
      course: "",
      userCourses: [],
  ...
  methods: {
    ...
    addTestTemplate() {
      this.$axios.post('eval/add-test-template', {
        title: this.title,
