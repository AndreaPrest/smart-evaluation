<template>
  <div>
    <validation-observer
      ref="observer"
      v-slot="{ handleSubmit }">
      <form @submit.prevent="handleSubmit(editSQL)">
        ...
        <h3>Snapshot Data
          ...
          <v-btn v-if="databases.length>1" icon @click="removeLastDatabase"><v-icon>mdi-minus</v-icon></v-btn>
        </h3>
        <div v-for="(database,index) in databases" :key="index">
          <h4>Snapshot Data n° {{ index + 1 }}</h4>
            <validation-provider v-slot="{ errors }" name="table" rules="required">
              <prism-editor v-model="databases[index]" :highlight="highlighter" line-numbers class="my-editor" width="100%"></prism-editor>
              <p style="color: red">{{ errors[0] }}</p>
            </validation-provider>
        </div>
...
export default {
  ...
  data() {
    return {
      query: this.sqlToEdit.content.query,
      databases: this.sqlToEdit.content.snapshotTables,
      results: [],
      queryErrors: [],
      showResults: false,
      db: new SQL.Database(),
      title:this.sqlToEdit.title,
      userCourses: [],
      topics: [],
      selectedTopics: this.getSelectedTopicsNames(this.sqlToEdit.topics),
      selectedCourses: this.getSelectedCoursesNames(this.sqlToEdit.courses),
      questionText: this.sqlToEdit.question,
      alert: false,
      errorMessage:"",
      errorType: "error",
      databaseSchema: this.sqlToEdit.content.databaseSchema,
      sameColumnRelevant:this.sqlToEdit.content.sameColumnRelevant,
      orderRelevant:this.sqlToEdit.content.orderRelevant,
      multiplicityRelevant:this.sqlToEdit.content.multiplicityRelevant
    }
  },
  computed: {
    topicsNames() {
  ...
  methods: {
    ...
    getSelectedCoursesNames(selectedCourses)
    {
      return selectedCourses.map(v => v.name)
    },
    getSelectedTopicsNames(selectedTopics)
    {
      return selectedTopics.map(v => v.name)
    },
    ...
    editSQL()
    {
      this.$axios.post('eval/edit-question', {
        _id: this.sqlToEdit._id,
        title:this.title,
        question: this.questionText,
        content: {snapshotTables: this.databases,query: this.query,testPassed: 0,databaseSchema: this.databaseSchema, orderRelevant: this.orderRelevant, sameColumnRelevant: this.sameColumnRelevant, multiplicityRelevant: this.multiplicityRelevant},
    ...
    testQuestion()
    {
      ...
      {
        this.resetDB()
        this.generateDB(this.databaseSchema + this.databases[i])
