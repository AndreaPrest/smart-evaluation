<template>
  <div>
    <validation-observer
      ...
      v-slot="{ handleSubmit }"
    >
      <form @submit.prevent="handleSubmit(editQuiz)">
...
name: "EditQuiz",
  ...
  data() {
    return {
      title: this.quizToEdit.title,
      selectedTopics: this.getSelectedTopicsNames(this.quizToEdit.topics),
      selectedCourses: this.getSelectedCoursesNames(this.quizToEdit.courses),
      questionText: this.quizToEdit.question,
      answers: this.quizToEdit.content.answers,
      correctAnswers: this.quizToEdit.content.correctAnswers,
      creator: this.quizToEdit.creator,
      quizId: this.quizToEdit._id,
      alert: false,
      errorMessage:"",
      errorType: "error",
      userCourses: [],
      ...
      this.$axios.post('eval/edit-question', {
        ...
        switch(code){
          case 409:
            this.$refs.observer.reset()
            this.clearFields()
            this.showAlert("Quiz Not Present in the Database","error")
            break;
          case 1062:
            this.$refs.observer.reset()
            this.clearFields()
