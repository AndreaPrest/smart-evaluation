    ...
    <validation-observer
      ...
      <form @submit.prevent="handleSubmit(addCode)">
        ...
        <h3>Tests
          ...
          <v-btn v-if="tests.length > 1" icon @click="removeTest"> <v-icon small>mdi-minus</v-icon></v-btn></h3>
...
export default {
  ...
  props: {
    ...
    data() {
      return {
        skeleton: this.codeToEdit.content.skeleton,
        tests: this.codeToEdit.content.tests,
        testLibraries: this.codeToEdit.content.testLibraries,
        result: "",
        reference: "/*try your implementation here*/",
        compileCodeResult: [],
        compileTestsResult: [],
        title:this.codeToEdit.title,
        userCourses: [],
        topics: [],
        selectedTopics: this.getSelectedTopicsNames(this.codeToEdit.topics),
        selectedCourses: this.getSelectedCoursesNames(this.codeToEdit.courses),
        questionText: this.codeToEdit.question,
        alert: false,
        errorMessage:"",
        errorType: "error",
      }
    ...
    computed: {
      ...
      completeCode() {
        return this.fixedLibraries + this.fixedReference  + this.fixedTests
      },
      completeTests() {
    ...
    methods: {
      ...
      compileTests() {
        ...
        {
          this.$axios.post('eval/compile-code', {
              code: this.fixedLibraries +  this.fixedSkeleton +  this.completeTests[i],
              language: "cpp17"
            }
          ).then((res)=>{
